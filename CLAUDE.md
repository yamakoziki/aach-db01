...
- `extract_personal()`: Splits `メンバー` field on whitespace, normalizes each token with `normalize_name()`, then partial-matches against one or more names (OR logic for variant spellings)
